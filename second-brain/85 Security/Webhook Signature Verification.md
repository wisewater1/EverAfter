...
**Fitbit.** `supabase/functions/webhook-fitbit/index.ts` verifies HMAC-SHA1/base64 (`supabase/functions/_shared/connectors.ts:78`), then — unlike push-payload providers — calls back to the Fitbit API with the user's stored OAuth token to fetch the actual data ([[Fitbit Integration]]).
...
- **Fitbit's GET verification handshake echoes instead of checking.** `supabase/functions/webhook-fitbit/index.ts:20-30` returns any `?verify=` value straight back with a 200. Fitbit's documented subscriber verification expects the endpoint to *compare* the code and answer 204 (match) or 404 (mismatch), so as written the handshake does not validate anything — and a 200-with-body is not what Fitbit looks for.
- **Dexcom webhook assumes a single connection.** `supabase/functions/cgm-dexcom-webhook/index.ts:32-36` looks up `connector_tokens` by `connector_id = 'dexcom'` alone — no user scoping, `.maybeSingle()`. With more than one connected Dexcom user this query breaks (or lands readings on the wrong account). Fine for the current single-user reality, a landmine at scale.
