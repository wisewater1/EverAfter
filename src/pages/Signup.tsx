...
export default function Signup() {
  ...
  return (
    <div className="min-h-[100dvh] bg-gradient-to-br from-gray-900 via-gray-800 to-blue-900 flex items-center justify-center p-3 sm:p-4 animate-page-enter">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-5 sm:mb-8">
          ...
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-light text-white mb-1 sm:mb-2">Create Account</h1>
          <p className="text-sm sm:text-base text-gray-400">Start building your personal AI today</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            {error && (
              <div className="bg-red-900/30 border border-red-500/50 rounded-lg p-3 sm:p-4 flex items-start gap-2 sm:gap-3">
                ...
                <div>
                  <p className="text-xs sm:text-sm text-red-400 font-medium">Signup Error</p>
              ...
              {password && (
                <div className="mt-2">
                  ...
                  <p className="text-xs text-gray-400">
                    Strength: <span className={strength >= 4 ? 'text-green-400' : strength >= 3 ? 'text-blue-400' : strength >= 2 ? 'text-yellow-400' : 'text-red-400'}>{strength > 0 ? strengthLabels[strength - 1] : 'Very Weak'}</span>
                  </p>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                {confirmPassword && password === confirmPassword && (
                  <CheckCircle className="absolute right-3 sm:right-4 top-1/2 transform -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-green-400" />
                )}
              </div>
            </div>
            ...
            <div className="flex items-start">
              <input
                type="checkbox"
                required
                className="w-3.5 h-3.5 sm:w-4 sm:h-4 mt-0.5 sm:mt-1 bg-gray-900 border-gray-700 rounded text-blue-600 focus:ring-blue-500/50"
              />
        ...
        {/* Footer */}
        <div className="mt-5 sm:mt-8 text-center text-xs sm:text-sm text-gray-500 px-2">
          <p>Free access &middot; No credit card required</p>
        </div>
      </div>
    </div>
  );
